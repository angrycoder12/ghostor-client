...
public final class StringBox extends RenderableSetting {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
		...
		String value = setting.getValue().length() <= 16 ? setting.getValue() : setting.getValue().substring(0, 16) + "…";
		int valueX = parentX() + parentWidth() - 20 - TextRenderer.getWidth(value);
		GhostorTheme.panel(context, valueX - 7, parentY() + parentOffset() + offset + 6, parentX() + parentWidth() - 13, parentY() + parentOffset() + offset + parentHeight() - 6, GhostorTheme.SURFACE_HOVER, 4);
		TextRenderer.drawString(value, context, valueX, (parentY() + parentOffset() + offset) + 9, GhostorTheme.TEXT_MUTED.getRGB());
        ...
        if(isHovered(mouseX, mouseY) && button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
            mc.setScreen(new Screen(Text.empty()) {
                ...
                public void render(DrawContext context, int mouseX, int mouseY, float delta) {
					...
					GhostorTheme.panel(context, startX, startY, startX + width, screenMidY + 36, GhostorTheme.SURFACE, GhostorTheme.RADIUS);
					GhostorTheme.outline(context, startX, startY, startX + width, screenMidY + 36, GhostorTheme.BORDER, GhostorTheme.RADIUS);
					TextRenderer.drawCenteredString(setting.getName(), context, screenMidX, startY + 10, GhostorTheme.TEXT.getRGB());
					GhostorTheme.panel(context, startX + 10, screenMidY, startX + width - 10, screenMidY + 30, GhostorTheme.SURFACE_ELEVATED, 5);
					GhostorTheme.outline(context, startX + 10, screenMidY, startX + width - 10, screenMidY + 30, GhostorTheme.ACCENT, 5);

					TextRenderer.drawString(content, context, startX + 15, screenMidY + 8, GhostorTheme.TEXT.getRGB());
