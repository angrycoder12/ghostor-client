...
public final class ModeBox extends RenderableSetting {
	...
	public void render(DrawContext context, int mouseX, int mouseY, float delta) {
		...
		TextRenderer.drawString(setting.getName(), context, nameOffset, (parentY() + parentOffset() + offset) + 9, GhostorTheme.TEXT.getRGB());
		String mode = setting.getMode().name();
		int modeX = parentX() + parentWidth() - 20 - TextRenderer.getWidth(mode);
		GhostorTheme.panel(context, modeX - 7, parentY() + parentOffset() + offset + 6, parentX() + parentWidth() - 13, parentY() + parentOffset() + offset + parentHeight() - 6, GhostorTheme.SURFACE_HOVER, 4);
		TextRenderer.drawString(mode, context, modeX, (parentY() + parentOffset() + offset) + 9, GhostorTheme.ACCENT_HOVER.getRGB());
